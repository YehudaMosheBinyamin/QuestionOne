...
int main(void)
{
    ...
    //Important note: if the computer's security settings\antivirus settings  allow changing the contents of files in Desktop,
    // the folowing test will fail and the function will return SUCCESSFUL_OPERATION 
    assert(ChangeContents("C:\\Users\\danbe\\Desktop\\NewDocument.txt") == FAILED_OPERATION);
